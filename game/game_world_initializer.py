...
def _link_rooms(forest_clearing: Room, manor: Room, foyer: Room, 
                dark_cave_entrance: Room, goblins_lair: Room, shack_shop: Room) -> None:
...
def _create_torch_interaction(forest_table: RoomObject) -> callable:
    ...
    def torch_on_table(val_hero, item: Item, *args):
        ...
        return ("You touch your torch to the prepared wood. The kindling catches immediately, "
...
def _populate_room_items(forest_clearing: Room, manor: Room, dark_cave_entrance: Room, goblins_lair: Room) -> None:
...
def _initialize_game_world() -> tuple[RpgHero, Room]:
    ...
    # Create rooms
    forest_clearing, manor, foyer, dark_cave_entrance, goblins_lair, shack_shop = _create_rooms()
    
    # Link rooms together
    _link_rooms(forest_clearing, manor, foyer, dark_cave_entrance, goblins_lair, shack_shop)
